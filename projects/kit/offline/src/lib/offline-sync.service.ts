...
export interface EnqueueOfflineCommand<T = unknown> {
  ...
  /** Consistency hint for an already replicated server identity; never creates a mapping by itself. */
...
export class OfflineSyncService {
  ...
  #initialServerId(current: number | null, incoming: number | null | undefined): number | null {
    if (incoming !== null && incoming !== undefined && (!Number.isSafeInteger(incoming) || incoming <= 0)) {
      throw new Error(`Offline command returned invalid serverId ${String(incoming)}.`);
    }
    // A caller-supplied id is only a consistency hint for a replica which is
    // already mapped. Never let a stale product-side read resurrect a
    // server-generated AUTO_INCREMENT id after a confirmed delete.
